...
public class NamespaceIterator implements AxisIterator {
            ...
            if (nslist != null) {
                for (int i=0; i<nslist.length; i++) {
                    ...
                    if (uriCode == 0) {
                        ...
                        undeclared.add(new Integer(prefixCode));
                    } else {
                        Integer key = new Integer(prefixCode);
                        if (!undeclared.contains(key)) {
                            declared.add(new Integer(nslist[i]));
                            undeclared.add(key);
                        }
                    }
                }
    ...
    public void advance() {
        while (nsIterator.hasNext()) {
            int nscode = ((Integer)nsIterator.next()).intValue();
    ...
    public int[] getInScopeNamespaceCodes() {
        ...
        int i = 0;
        while (nsIterator.hasNext()) {
            codes[i++] = ((Integer)nsIterator.next()).intValue();
        }
        return codes;
    }
    ...
    public static class NamespaceNodeImpl implements NodeInfo, FingerprintedNode {
        ...
        public AxisIterator iterateAxis(byte axisNumber) {
            return iterateAxis(axisNumber, new AnyNodeTest());
        }
        ...
        public String generateId() {
            return element.generateId() + 'n' + position;
        }
        ...
        public void copy(Receiver out, int whichNamespaces, boolean copyAnnotations, int locationId) throws XPathException {
            out.namespace(getNamePool().getNamespaceCode(getNameCode()), 0);
        }
